    ...
    query peopleSearchLoad {
      users (filter:{roles: ["STAFF","FACULTY","RETIREDSTAFF","RETIREDFACULTY"]}) {
          ...
          name {
            ...
            last
            prefixSpecial
            # pronouns - Adding to Motion, I think they'd go well as part of name.
          }
          campusEmail
          # jobTitle - Already pulling from AD in motion but not a queryable field yet.
          department
          # officeBldg - Adding pull of info from PYBEMPL.
          # officeRoom - Adding pull of info from PYBEMPL.
          officePhone
          # displayRole -  Needs to be fixed, there's a null value out there for someone.
          canLogin
      }
    }`)
