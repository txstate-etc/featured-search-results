...
class Helpers {
  ...
  static getWhereClause (tableDef, search) {
    const fieldTerms = this.getTerms(tableDef.hash).join('|')
    // Handle the most likely case that they send us a single token to query for.
    if (search.split(' ').length === 1) return ' where ' + (tableDef.defaults.map(field => `${field} like '%${search}%'`).join(' or '))
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      if (term) clause = `${tableDef.hash[term]} ${clause}`
      else clause = tableDef.defaults.map(field => `(${field} ${clause})`).join(' or ')// Didn't specify search term. Try quering against all SQL fields we search.
  ...
  static getSortClause (tableDef, sortOption) {
    ...
    return ' order by ' + ((sortOption && this.getFields(tableDef.hash).has(sortOption)) ? sortOption : sortDefault)
  }
