...
class Helpers {
  ...
  static sanitize (scrubMe) {
    /* If there are any containing single or double quote pairs strip them before
    |  passing to mysql's escape function that will add escapes to all dangerous characters.
    |  From there however, we'll need to strip any double or single quote delimiting pairs
    |  so we can insert our {sql wildcard: % } characters.
    */
    return db.pool.escape(scrubMe.replace(/^(["'])(.*?)\1$/, '$2')).replace(/^(["'])(.*?)\1$/, '$2')
  }
  ...
  static getWhereClause (tableDef, search) {
    ...
    if (search.split(' ').length === 1) return ' where ' + (tableDef.defaults.map(field => `${field} like '%${this.sanitize(search)}%'`).join(' or '))
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      console.log('likeop: ' + likeop, 'alias: ' + alias, 'wildcardop: ' + wildcardop, 'whatfor: ' + whatfor)
      // Start building our clause with what we're searching for stripped of any grouping quotes and escaping any " or ' in the string.
      let clause = this.sanitize(whatfor)
      ...
      else clause = tableDef.defaults.map(field => `(${field} ${clause})`).join(' or ')// Didn't specify search alias. Try quering against all SQL fields we search.
