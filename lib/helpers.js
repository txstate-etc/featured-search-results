...
class Helpers {
  ...
  static getWhereClause (tableDef, search) {
    ...
    if (search.split(' ').length === 1) {
      const sanitized = search.replace(/(?:^["']|["']$)/g, '').replace(/('|"|\\)/g, '\\$1') // Please don't allow SQL injection.
      return ' where ' + (tableDef.defaults.map(field => `${field} like '%${sanitized}%'`).join(' or '))
    }
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      let clause = whatfor.replace(/(?:^["']|["']$)/g, '').replace(/('|"|\\)/g, '\\$1') // Please don't allow SQL injection.
