...
class Helpers {
  ...
  static getPeopleHash() { 
    // There's probably a better way to do this. PLEASE REMEMBER TO ASK!!!!
    /* I set it up this way so we could extend this to different SQL data sources if we need to.
       getWhereClause will generate a where clause for any hash of terms=>fields in an sql table.
    */
  ...
  static getWhereClause(hash, search) {
    ...
    for ( const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      let clause = ''
      if (whatfor) { // Start building our clause with what we're searching for stripped of any grouping quotes.
        clause = clause + whatfor.replace(/(?:^["']|["']$)/g,'')
      }
      switch(true) { // Add SQL wildcard symbols per the term likeops, if specified.
        case /^(?:(?:begins|starts)\s?with|<)/.test(wildcardop): clause =     clause+'%'; break
        case              /^(?:ends\s?with|>)/.test(wildcardop): clause = '%'+clause    ; break
        case                 /^(?:contains|:)/.test(wildcardop): clause = '%'+clause+'%'; break
      //default /*           (is|=|undefined) - No wildcards */: clause =     clause
      }
      switch(true) {  // SQL query will search using like comparisons. The likeop determines negative or positive like.
        case /^(?:and\s+|\+)$/.test(likeop): clause =     `like '${clause}'`; break
        case  /^(?:not\s+|-)$/.test(likeop): clause = `not like '${clause}'`; break
        default /* likeop is undefined   */: clause =     `like '${clause}'` 
      }
      if (term) { // Prepend our term's SQL field name, if term is specified, else default to ALL.
        clause = hash[term]+' '+clause
      }
      else { // Didn't specify search term. Try quering against all SQL fields we search.
        let defaultclause = ''
        for (const distinct of fields) {
          defaultclause += `(${distinct} ${clause}) or `
        }
        clause = defaultclause.slice(0,-4) // Strip off the last ' or '.
      }
      whereClause += `(${clause}) and `
    }
    whereClause = whereClause.slice(0,-5) // Strip off the last ' and '.
