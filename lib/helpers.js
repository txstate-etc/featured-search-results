...
class Helpers {
  ...
  static getFields (hash) { // Get the values from the hash as the field names in a SQL table.
    return new Set(Object.values(hash)) // Set() to just the distinct field names.
  }

  static getAliases (hash) { // Get the keys from the hash as the aliases to the field names in a SQL table.
    return Object.keys(hash)
  }

  static getPeopleDef () {
    // type tableDef - Must define: { hash: {}, defaults [], fields: Set() }
    const peopleDef = { hash: {}, defaults: [], fields: '' }
    peopleDef.hash = {
      'lastname':  'lastname',
      'last name': 'lastname',
      'last':      'lastname',
    ...
    peopleDef.fields = this.getFields(peopleDef.hash)
    return peopleDef
  }
  ...
  static getWhereClause (tableDef, search) {
    ...
    |  Returns and object of {sql: string, binds: [string]}. Example: {sql: ' where field like ?', binds: ['%whatfor%']}
    */
    const binds = []
    const fieldAliases = this.getAliases(tableDef.hash).join('|')
    // We need to break the RegEx down to interpolate the fieldAliases. While wße're at it might as well tokenize
    ...
    for (const token of tokens) {
      ...
      // Redo this to make like the obvious default with not like being tested for.
      ...
      else clause = tableDef.defaults.map(field => (binds.push(wildcardbind) && `(${field} ${clause})`)).join(' or ')
      whereClause.push(`(${clause})`)
    }
    return { sql:' where ' + whereClause.join(' and '), binds: binds }
  }

  static getSortClause (tableDef, ...sortOptions) {
    ...
    */
    ...
    if (sortOptions.length === 0) return ` order by ${sortDefault} asc`

    const sortClause = []
    for (const optionToken of sortOptions) {
      if (!optionToken) ; // Ignore this token if it's null or undefined.
        ...
        for (const fieldOp of fieldOps) {
          if (tableDef.fields.has(fieldOp)) sortClause.push(fieldOp + ((optionToken.order && optionToken.order.match(/^(?:d|desc)/i)) ? ' desc' : ' asc'))
        }
    ...
    return 'order by ' + (sortClause.length === 0 ? sortDefault + ' asc' : sortClause.join(', '))
  }

  static getLimitClause (pageNum, pageSizes) {
    ...
    |  Returns a string. Example: ' limit by 12, 3'
    */
    const limitDefault = ''
