...
const pool = require('mysql2-async/db').default.pool
...
class Helpers {
  ...
  static sanitize (scrubMe) {
    ...
    |  but also include the containing quotes we want to strip.
    */
    return pool.escape(scrubMe)
  }
  ...
  static getWhereClause (tableDef, search) {
    ...
    if (search.split(' ').length === 1) {
      const whatfor = search.replace(/^(["'])(.*?)\1$/, '$2')
      return ' where ' + tableDef.defaults.map(field => field + ' like ' + this.sanitize(`%${whatfor}%`)).join(' or ')
    }
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      let clause = whatfor.replace(/^(["'])(.*?)\1$/, '$2')
      ...
      if (!likeop || /^(?:and\s+|\+)$/.test(likeop)) clause =     `like ${this.sanitize(clause)}`
      else if (      /^(?:not\s+|-)$/.test(likeop))  clause = `not like ${this.sanitize(clause)}`
