...
class Helpers {
  ...
  static getWhereClause (tableDef, search) {
    ...
    if (search.split(' ').length === 1) {
      ...
      return {
        sql: ' where ' + tableDef.defaults.map(field => field + ' like ?').join(' or '),
        binds: tableDef.defaults.map(field => `%${whatforbind}%`)
      }
    }
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      if (!wildcardop || /^(?:(?:begins|starts)\s?with|<)/.test(wildcardop)) binds.push( `${whatforbind}%`)
      else if (                       /^(?:ends\s?with|>)/.test(wildcardop)) binds.push(`%${whatforbind}` )
      else if (                          /^(?:contains|:)/.test(wildcardop)) binds.push(`%${whatforbind}%`)
      else if (                                /^(?:is|=)/.test(wildcardop)) binds.push(    whatforbind   )
