...
class Helpers {
  ...
  static getWhereClause (tableDef, search) {
    ...
    for (const token of tokens) { // Build our where clause parts out of the tokens building right to left within each part.
      ...
      if (alias) {
        clause = `${tableDef.hash[alias]} ${clause}`
        binds.push(wildcardbind)
      } else {
        clause = tableDef.defaults.map(field => `(${field} ${clause})`).join(' or ')// Didn't specify search alias. Try quering against all SQL fields we search.
        tableDef.defaults.map(field => binds.push(wildcardbind))
      }
