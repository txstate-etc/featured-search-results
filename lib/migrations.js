...
async function cloneSearchIdTableData () {
  ...
  // const warehouseRoles = await warehouseDB.query('SELECT true')
  // console.log(warehouseRoles)
  ...
  */
  ...
  const exclusionsTableDDL = `
    ...
    // Associated aNumbers to be excluded can be found in the PPLSEACH_EXCLUDE field of Ansible's Production Warehouse group vars and used
    // to query for the userids to plug in to manual insert statement below.
    // INSERT INTO special_exclusions (userid) VALUES ('1'), ('2'), ('3'), ('4');
...
module.exports = async function () {
  ...
  if (tables.some(table => table === 'people')) {
    ...
    if (compareResult) {
      ...
      const renameResult = await db.query(`RENAME TABLE people TO ${recoveryName}`)
      const createResult = await db.query(peopleTableDDL)
      const dropResult = await db.query(`DROP TABLE IF EXISTS ${recoveryName}`)
    }
  } else {
  ...
  if (tables.some(table => table === 'special_exclusions')) {
    ...
    const createExclusionsResult = await db.query(exclusionsTableDDL)
  }
